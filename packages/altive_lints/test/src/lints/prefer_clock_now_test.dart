...
import 'package:altive_lints/src/fixes/replace_datetime_with_clock_now.dart';
...
import 'package:analysis_server_plugin/src/correction/fix_processor.dart'; // ignore: implementation_imports
...
''');
  ...
  Future<String> _applyFix(String content) async {
    ...
    final fix = fixes.singleWhere(
      (fix) => fix.change.id == 'altive_lints.fix.replaceDateTimeWithClockNow',
    );
...
''';
    ...
    expect(
      fixes.where(
        (fix) =>
            fix.change.id == 'altive_lints.fix.replaceDateTimeWithClockNow',
      ),
      isEmpty,
    );
  }
